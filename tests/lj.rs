...
// [[file:~/Workspace/Programming/rust-scratch/fire/fire.note::*fire][fire:1]]
#[test]
fn test_fire_opt() {
    use vecfx::*;
    use fire::fire::FIRE;
    use fire::fire::{Termination, UserTermination};
    ...
    opt.minimize(
        ...
        UserTermination::new(|prgr| {
            println!(
                "niter = {:5}: fx = {:-10.4}, gnorm = {:-10.4}",
                prgr.niter, prgr.fx, prgr.gnorm
            );
            if prgr.niter > 1000 || prgr.gnorm <= 0.1 {
                true
            } else {
                false
            }
